...
const Booking = () => {
    ...
    const tourData = data?.[0];
      ...
      useEffect(() =>{
        if(tourData) {
            const amount = tourData?.costFrom * guestCount;
            setTotalAmount(amount);
        } 
      }, [guestCount, totalAmount, tourData])
      ...
      const handleBooking = async () => {
        ...
        try {
           ...
           window.location.href = res.paymentUrl;
            toast.success("Booking done", { id: toastId });
        } catch (error: any) {
            toast.error(error?.data?.message || "Failed to booking the tour", { id: toastId });
        }
      }
